...
function meals(){
  ...
  $("catalogLibrary").innerHTML=window.__catNames.map((name,i)=>{const row=CATALOG.find(x=>x.meal_name===name&&x.gender===user().gender)||uniq[name];return `<div class="list-item"><span><b>${esc(name)}</b><br><small>Week ${n(row.week)} • Meal ${n(row.meal)} • ${n(row.kcal)} kcal • P ${n(row.protein_g)}g • C ${n(row.carbs_g)}g • F ${n(row.fat_g)}g</small></span><button class="danger" onclick="deleteCatalogMeal(${i})">Delete</button></div>`}).join("")||'<p class="muted">No catalog meals yet. Add one above.</p>';
}
function planView(){
